...
export async function POST(req: Request) {
    try {
        ...
        if (!address) return NextResponse.json({ success: false });
        ...
        for (let i = 1; i < Number(nextAgentId); i++) {
            ...
            if (walletAddress.toLowerCase() === target) {
                ...
                if (nameBytes !== '0x' && nameBytes) {
                    const agentName = Buffer.from(nameBytes.slice(2), 'hex').toString('utf8');
                    return NextResponse.json({ success: true, name: agentName + ".pvparena.eth" });
                }
            }
        }
        ...
        return NextResponse.json({ success: false, error: 'Reverse resolution failed' }, { status: 500 });
    }
}
